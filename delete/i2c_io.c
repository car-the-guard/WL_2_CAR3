...
static const unsigned char font8x8[][8] = {
    ...
    ['3'] = {0x22, 0x42, 0x49, 0x49, 0x49, 0x36, 0x00, 0x00},
...
void LCD_display_v2x_mode(int mode, uint32_t sid, uint64_t aid, uint16_t dist, uint8_t lane, uint8_t type) {
    ...
    // LCD 텍스트 구성
    //if (mode == 2) { sprintf(l1, "!!! WARNING !!!"); sprintf(l2, "DIST:%dm", dist); }
    if (mode == 2) { 
    ...
    // Line 2: 릴레이를 해준 차량의 ID와 거리를 출력
    // 'relay_id'는 수신한 패킷의 pkt->sender_id (차2의 ID인 0x2222)입니다.
    sprintf(l2, "  RECV FR:%04X ", sid); 
    }
